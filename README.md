...
- **A believable calendar.** Business years and periods, seasonality by season, a weekday curve (Saturday sells about 53% more than a Tuesday), and the key trading days: Black Friday at about twice a normal Friday, Boxing Day at about 1.5 times a normal day, and every store shut on Christmas Day while Online keeps trading.
- **Hero styles and a long tail.** Each style has its own popularity weight, so about a fifth of the styles do around 60% of sales, the way a real range behaves.
...
- **Brand share.** I wanted my lead brand to have about a third of sales *and* to supply most of the top-20 styles. I found that a single demand multiplier can't do both, because a brand with more styles has a higher best-seller purely from the maths of picking the maximum, however its total share turns out. I ended up with Areta at about 34% of sales and 14 of the top 20, and always re-check the real top 20 after touching the multiplier, because it's very sensitive to small changes. Adding style popularity later reshuffled the top 20 to 10 Areta styles across seven product groups, which is more variety than I'd managed with multipliers alone, and the brand shares didn't move.
...
- **A Pareto that wasn't one.** My style Pareto came out nearly flat: it took 55% of styles to reach 80% of sales, and the best style sold only four times the median. The generator had no idea of a popular style, so every style in the same brand and product group sold at the same rate apart from price. I gave each style a random popularity weight from a skewed distribution, tested a few spreads against the real totals first, and rescaled the weights inside every brand and product group so the mix I'd already calibrated stayed put. Now 36% of styles make up 80% of sales.
...
**15. The data wasn't as realistic as I thought.** Querying it in SQL showed Black Friday sales at almost exactly 1.00 times a normal day and return rates of about 3% everywhere. Both are things a real retailer wouldn't have. The cause was more interesting than a missing flag. The promo model lifts units by a fixed factor of the discount, and at 40% off that doesn't make up for the lower price, so Black Friday actually came out slightly *below* a normal Friday, and the Boxing Day sale well below. Real retailers get those days from a surge in shoppers, not the discount alone, so I added a demand multiplier for each key day, set from the revenue I wanted to see rather than guessed, and shut the stores on Christmas Day. A calendar heatmap in Power BI was what finally made it obvious, and it's also how I checked the fix. Return rates are still about 3% everywhere, which I've noted rather than hidden. The same went for margin: I'd calibrated a range per discount depth but held it constant, so every business year came out at the same 69.4% and every margin year-on-year figure read 0.0. I added a small drift over time (about -0.5 points in 2024, then +0.3 in 2025), which changes only the cost on each sale and nothing else, so the units, prices and sales are exactly as they were.
